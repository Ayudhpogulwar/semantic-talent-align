 ...
 * AdminLogin.jsx (Super Admin Authentication & Provisioning Portal)
 */
...
export default function AdminLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isSignUp) {
        // Show assigned secret key to admin after registration
  ...
  return (
    <div className="d-flex align-items-center justify-content-center min-vh-100 py-4 px-3" style={{ background: "var(--bg-dark, #0b0f19)" }}>
      {/* Compact Box: maxWidth 380px */}
      <div 
        className="glass-panel p-3.5 p-sm-4 text-center" 
        style={{ 
          maxWidth: 380, 
          width: "100%", 
          borderRadius: "16px", 
      ...
      >
        ...
        <div className="mb-3">
          ...
          <h4 className="fw-bold text-white mb-1 fs-5">{isSignUp ? "Super Admin Register" : "Super Admin Login"}</h4>
          <p className="text-secondary mb-0" style={{ fontSize: "0.75rem" }}>SAIOTAF Framework • Command Center</p>
        </div>
        ...
        {registeredKey ? (
          <div className="text-start py-2">
            ...
            <div className="p-3 mb-3 rounded-3 text-center" style={{ background: "rgba(0,0,0,0.4)", border: "1px solid rgba(234, 179, 8, 0.3)" }}>
              <small className="text-muted d-block mb-1" style={{ fontSize: "0.7rem", uppercase: "true" }}>Your Secret Access Key</small>
          ...
          <>
            ...
            <div className="d-flex p-1 mb-3 rounded-3" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmit} className="text-start">
              ...
              {isSignUp && (
                <div className="mb-2.5">
                  <label className="form-label text-secondary fw-semibold mb-1" style={{ fontSize: "0.78rem" }}>Admin Full Name</label>
              ...
              {/* Admin Email */}
              <div className="mb-2.5">
                <label className="form-label text-secondary fw-semibold mb-1" style={{ fontSize: "0.78rem" }}>Admin Email Address</label>
                <div className="position-relative">
                  <input
                    type="email"
                    className="form-control faculty-search-input ps-4"
              ...
              {/* Password */}
              <div className="mb-2.5">
                <label className="form-label text-secondary fw-semibold mb-1" style={{ fontSize: "0.78rem" }}>Password</label>
                <div className="position-relative">
                  <input
                    type="password"
                    className="form-control faculty-search-input ps-4"
              ...
              {/* Secret Access Key */}
              <div className="mb-3">
                <label className="form-label text-secondary fw-semibold mb-1 d-flex align-items-center justify-content-between" style={{ fontSize: "0.78rem" }}>
                  <span>Secret Access Key</span>
                  <span className="badge px-2 py-0.5 fw-bold rounded-pill" style={{ background: "rgba(244, 63, 94, 0.15)", color: "#fb7185", border: "1px solid rgba(244, 63, 94, 0.35)", fontSize: "0.65rem" }}>
                    8 Chars Required
                  </span>
                </label>
                <div className="position-relative">
                ...
                <small className="text-secondary mt-1 d-block" style={{ fontSize: "0.72rem" }}>
                  {isSignUp 
              ...
              <button
                type="submit"
                className={`btn ${isSignUp ? 'btn-primary' : 'btn-danger'} w-100 py-2 fw-bold d-flex align-items-center justify-content-center gap-2 mb-2.5`}
                disabled={loading}
                style={{ fontSize: "0.85rem" }}
              >
                {loading
                  ? (isSignUp ? "Creating Super Admin…" : "Verifying Credentials…")
                  : (isSignUp ? "Register Super Admin Account" : "Sign In to Console")}
              </button>
            </form>

            {/* Footer Links */}
            <div className="border-top border-secondary pt-2.5 mt-2 d-flex flex-column align-items-center gap-1.5">
              <button
                type="button"
              ...
              >
                {isSignUp ? "Already registered? Sign In" : "Need a new admin account? Register"}
              </button>
              
              <Link to="/" className="text-secondary text-decoration-none d-inline-flex align-items-center gap-1" style={{ fontSize: "0.75rem" }}>
                <ArrowLeft size={13} /> Return to Main Landing Page
              </Link>
            </div>
          </>
        )}
